import { source, type PIPPage } from '@/lib/source';
...
// PIP Index/Overview Page Component
function PIPIndexPage() {
  const categories = source.getCategorizedPages();
  const stats = source.getStats();

  return (
    <div className="max-w-4xl mx-auto">
      <h1 className="text-3xl font-bold mb-4">All Pars Improvement Proposals</h1>
        ...
        <section key={cat.name} className="mb-12">
          ...
          >
            ...
            <span className="text-xs text-muted-foreground px-2 py-1 rounded-full bg-accent group-hover:bg-primary/10 transition-colors">
              {cat.pips.length} proposals
            </span>
            <ArrowRight className="size-4 text-muted-foreground opacity-0 group-hover:opacity-100 transition-opacity" />
          </Link>
          <p className="text-sm text-muted-foreground mb-4">{cat.shortDesc}</p>
              ...
              >
                ...
                {pip.data.frontmatter.status && (
                  <span className={`text-xs px-2 py-0.5 rounded-full shrink-0 ${
                    pip.data.frontmatter.status === 'Final' ? 'bg-primary/10 text-primary' :
                    pip.data.frontmatter.status === 'Draft' ? 'bg-yellow-500/10 text-yellow-500' :
                    pip.data.frontmatter.status === 'Review' ? 'bg-blue-500/10 text-blue-500' :
...
// Individual PIP Page Component
function PIPDetailPage({ page }: { page: PIPPage }) {
        ...
        <div className="flex items-start justify-between gap-4 mb-4">
          <div>
            <span className="text-sm font-mono text-muted-foreground">
              PIP-{String(frontmatter.pip).padStart(4, '0')}
            </span>
...
export default async function Page({
  ...
  return <PIPDetailPage page={page} />;
}
...
export async function generateMetadata({ params }: { params: Promise<{ slug?: string[] }> }) {
  ...
  if (!slug || slug.length === 0) {
    return {
      title: 'All Proposals',
      description: 'Browse all Pars Improvement Proposals (PIPs) - standards for the Pars Network ecosystem',
    };
  }
  ...
  return {
    title: `PIP-${page.data.frontmatter.pip}: ${page.data.title}`,
    description: page.data.description || `Pars Improvement Proposal ${page.data.frontmatter.pip}`,
  };
}
